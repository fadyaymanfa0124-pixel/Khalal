...
const setTrackNameWithFade = (title) => {
  if (trackNameElement.textContent === title && !trackNameElement.classList.contains("is-changing")) {
    return;
  }

  if (trackNameAnimationFrame) {
    window.clearTimeout(trackNameAnimationFrame);
  }

  trackNameElement.classList.add("is-changing");

  trackNameAnimationFrame = window.setTimeout(() => {
    trackNameElement.textContent = title;
    trackNameElement.classList.remove("is-changing");
  }, 180);
};

const updateTrackMeta = (message) => {
  if (!hasPlayableTracks) {
    setTrackNameWithFade("Waiting For Tracks");
    trackStatusElement.textContent = "ابعتي روابط أو ملفات MP3 للتراكات وسأربطها فوراً. حالياً الروابط الخارجية فقط شغالة.";
    return;
  }

  setTrackNameWithFade(albumData.tracks[currentTrackIndex]?.title || "Random Track");
